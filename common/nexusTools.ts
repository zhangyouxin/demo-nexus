...
export const getOffChainLocks = async (
  ...
  return ckb.request({ method: 'wallet_fullOwnership_getOffChainLocks', params: {} });
};
...
export const getOnChainLocks = async (
  ...
  const locks: Script[] = [];
  // get first page
  let onChainInfos = await ckb.request({ method: 'wallet_fullOwnership_getOnChainLocks', params: { change: type } });
  ...
  while (onChainInfos.objects.length === DEFAULT_NEXUS_PAGE_SIZE) {
    onChainInfos = await ckb.request({ method: 'wallet_fullOwnership_getOnChainLocks', params: { cursor: onChainInfos.cursor, change: type } });
    locks.push(...onChainInfos.objects);
  }
  return locks;
};
...
export const getAllLiveCells = async (ckb: typeof window.ckb): Promise<Cell[]> => {
  ...
  let liveCellsResult = await ckb.request({ method: 'wallet_fullOwnership_getLiveCells', params: {} });

  fullCells.push(...liveCellsResult.objects);
  // if the first page has DEFAULT_NEXUS_PAGE_SIZE items, then there are more pages, nexus default page size is DEFAULT_NEXUS_PAGE_SIZE
  while (liveCellsResult.objects.length === DEFAULT_NEXUS_PAGE_SIZE) {
    liveCellsResult = await ckb.request({ method: 'wallet_fullOwnership_getLiveCells', params: { cursor: liveCellsResult.cursor } });
